...
export interface LipScrollZoominAnimationProps {
  ...
  /** Faint word rendered behind the mask. */
  watermark?: string
  videoSrc?: string
  posterSrc?: string
  className?: string
...
export function LipScrollZoominAnimation({
  ...
  useEffect(() => {
    ...
    const setMaskSize = (size: number) => {
      ...
      if (!el) return
      el.style.setProperty('--maskW', `${size}px`)
      el.style.webkitMaskSize = `${size}px`
      el.style.maskSize = `${size}px`
    }

    // index.css kills CSS animation under reduced motion, but GSAP scrub is
    // JS-driven — opt out explicitly rather than pinning the viewport.
    const reduceMotion =
      ...
      const tl = gsap.timeline({
        scrollTrigger: {
          ...
          onUpdate: (self) => {
            ...
            setMaskSize(startSize + Math.pow(self.progress, 2.3) * 4500)
          },
        },
      })
  ...
  return (
    <div
      className={`lip-scroll-root w-full bg-white text-black selection:bg-blue-600 selection:text-white ${className}`}
    >
      {/* 1. INTRO SECTION */}
      ...
      >
        <div
          ref={pinRef}
          className="motion-section__pin sticky top-0 w-full h-screen overflow-hidden flex items-center justify-center bg-white select-none"
        >
          {/* Top-Left Corner */}
          <div className="absolute top-[10px] left-[10px] z-30 pointer-events-none w-4 h-4 sm:w-5 sm:h-5 text-black">
            <svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" viewBox="0 0 10 10" fill="none" className="w-full h-full" aria-hidden="true">
          ...
          <div className="absolute inset-0 flex items-center justify-center opacity-[0.03] pointer-events-none select-none z-0">
            <span className="text-[20vw] font-black uppercase tracking-tighter text-black">{watermark}</span>
          </div>
      ...
      <style
        dangerouslySetInnerHTML={{
          __html: `
            .lip-scroll-root,
            .lip-scroll-root p,
            .lip-scroll-root span,
            .lip-scroll-root h1,
            .lip-scroll-root h2 {
              font-family: 'Poppins', 'Inter', ui-sans-serif, system-ui, sans-serif;
              font-style: normal;
            }
            .lip-scroll-root .pin-spacer {
